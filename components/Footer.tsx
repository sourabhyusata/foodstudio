...
export default async function Footer() {
  ...
  const addressShort = settings.address_short || '';
  const openingHours = settings.opening_hours || '';
  const whatsappNumber = settings.whatsapp_number || '';
  const instagramUrl = settings.instagram_url || '';
  const zomatoUrl = settings.zomato_url || '';
  const swiggyUrl = settings.swiggy_url || '';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <ul className="space-y-3 text-sm text-gray-300">
              ...
              {phoneNumber && (
                <li className="flex items-center gap-2">
                  ...
                  <a href={`tel:${phoneNumber.replace(/\s/g, '')}`} className="hover:text-saffron-light">
                    {phoneNumber}
                  </a>
                </li>
              )}
              {email && (
                <li className="flex items-center gap-2">
                  ...
                  <a href={`mailto:${email}`} className="hover:text-saffron-light">
                    {email}
                  </a>
                </li>
              )}
              {openingHours && (
                <li className="flex items-start gap-2">
                  ...
                  <span>Open Daily: {openingHours}</span>
                </li>
              )}
            </ul>
          </div>
          ...
          <div>
            ...
            {whatsappNumber && (
              <a
                href={`https://wa.me/${whatsappNumber}?text=Hi%20Dosa%20Darbar!`}
                target="_blank"
                rel="noopener noreferrer"
