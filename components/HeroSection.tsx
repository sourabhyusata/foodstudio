...
export default function HeroSection({ settings = {} }: HeroSectionProps) {
  const badgeText = settings.hero_badge_text || '';
  ...
  const stats = [
    { value: settings.stat_dosa_varieties || '', label: 'Dosa Varieties' },
    { value: settings.stat_happy_customers || '', label: 'Happy Customers' },
    { value: settings.stat_zomato_rating || '', label: 'Zomato Rating' },
  ].filter((s) => s.value);

  // Split title to highlight "Dosa" in accent color
  const titleParts = title.split('Dosa');
